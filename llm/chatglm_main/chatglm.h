...
class BaseTokenizer {
  public:
    ...
    virtual std::pair<ov::Tensor, ov::Tensor> encode(const std::string &text, int max_length) const = 0;

    virtual std::string decode(const std::vector<int> &ids) const = 0;

    virtual std::pair<ov::Tensor, ov::Tensor> encode_messages(const std::vector<ChatMessage> &messages, int max_length) const = 0;
...
class TextStreamer : public BaseStreamer {
  public:
    TextStreamer(std::ostream &os, BaseTokenizer *tokenizer)
        : os_(os), tokenizer_(tokenizer), is_prompt_(true), is_first_line_(true), print_len_(0), py_streamer(false) {}
    void put(const std::vector<int> &output_ids) override;
    void end() override;
    bool py_streamer;
    std::string py_userData;
    void registerCallBack(std::function<void(std::string, std::vector<int>)>& handler);
    std::function<void(std::string, std::vector<int>)> py_handler = nullptr;
...
class PerfStreamer : public BaseStreamer {
  public:
    ...
    void end() override {
        ...
        end_us_ = std::chrono::duration_cast<std::chrono::microseconds>(currentTime.time_since_epoch()).count();
    }
...
class BaseModelForCausalLM {
  public:
    BaseModelForCausalLM(ov::Core core, const std::string openvino_model_path, const std::string device, ov::AnyMap device_config);
    ...
    int64_t get_out_token_id(const std::vector<int>& input_ids, float* logits, size_t vocab_size, const GenerationConfig& gen_config);

    std::vector<int> generate(const ov::Tensor input_ids, const ov::Tensor attention_mask, const GenerationConfig& gen_config, BaseStreamer* streamer = nullptr);
    
    int generate_next_token (const ov::Tensor input_ids, const GenerationConfig& gen_config,int n_past, int n_ctx);
    // logits processor
    static void sampling_repetition_penalty(float *first, float *last, const std::vector<int>& input_ids, float penalty);
    // logits warper
  ...
  public:
    ModelConfig config;
    std::vector<ov::Output<const ov::Node>> model_inputs; 
...
class ChatGLMTokenizer : public BaseTokenizer {
  public:
    ...
    ~ChatGLMTokenizer() = default;
    std::pair<ov::Tensor, ov::Tensor> encode(const std::string &text, int max_length) const override;

    std::string decode(const std::vector<int> &ids) const override;

    std::pair<ov::Tensor, ov::Tensor> encode_messages(const std::vector<ChatMessage> &messages, int max_length) const override;
...
class ChatGLM3Tokenizer : public BaseTokenizer {
  public:
    ChatGLM3Tokenizer(ov::Core core, const std::string& tokenizer_path, const std::string& detokenizer_path);
    ~ChatGLM3Tokenizer() = default;
//    std::vector<int> encode(const std::string &text, int max_length) const override;
    std::pair<ov::Tensor, ov::Tensor> encode(const std::string& text, int max_length) const override;
    std::string decode(const std::vector<int> &ids) const override;
    std::pair<ov::Tensor, ov::Tensor> encode_messages(const std::vector<ChatMessage> &messages, int max_length) const override;
  ...
  private:
    std::pair<ov::Tensor, ov::Tensor> tokenize( std::string&& prompt) const;
    std::string detokenize(std::vector<int64_t>& tokens) ;

    std::string encode_single_message(const std::string &role, const std::string &content) const;
    ...
    static void truncate(std::string result, int max_length);
  ...
  public:
    ...
    mutable ov::InferRequest ov_tokenizer;
    mutable ov::InferRequest ov_detokenizer;


};

//using ChatGLM3Model = ov::CompiledModel ;
...
class Pipeline {
  public:
    Pipeline(const std::string& model_path, const std::string ov_device);
    ~Pipeline() = default;
    std::vector<int> generate(const ov::Tensor input_ids, const ov::Tensor attention_mask, const GenerationConfig &gen_config,
    ...
    ChatMessage chat(const std::vector<ChatMessage> &messages, const GenerationConfig &gen_config,
                     BaseStreamer *streamer = nullptr) const;
    ChatMessage chat_pygenerate(const std::vector<ChatMessage>& messages, const GenerationConfig& gen_config);
    void chat_pystream(const std::vector<ChatMessage>& messages, const GenerationConfig& gen_config, std::function<void(std::string, std::vector<int>)> &cb , std::string userdata);
    void Pipeline::callback(void* userdata);

  public:
    std::unique_ptr<ChatGLM3Tokenizer> chatglm3_tokenizer;
