...
class PyBaseTokenizer : public BaseTokenizer {
  public:
    ...
    //    PYBIND11_OVERRIDE_PURE(std::pair<ov::Tensor, ov::Tensor>, BaseTokenizer, encode, text, max_length);
    //}
    std::string decode(const std::vector<int> &ids) const override {
          PYBIND11_OVERLOAD_PURE(std::string, BaseTokenizer, decode, ids);
    }
    //std::pair<ov::Tensor, ov::Tensor> encode_messages(const std::vector<ChatMessage> &history, int max_length) const override {
    //    PYBIND11_OVERLOAD_PURE(std::pair<ov::Tensor, ov::Tensor>, BaseTokenizer, encode_messages, history, max_length);
    //}
};
...
PYBIND11_MODULE(_C, m) {
   ...
   //py::class_<BaichuanTokenizer, BaseTokenizer>(m, "BaichuanTokenizer");
    ...
    //::class_<InternLM20BForCausalLM, BaseModelForCausalLM>(m, "InternLM20BForCausalLM");

    // ===== Pipeline ====

    py::class_<Pipeline>(m, "Pipeline")
        //.def(py::init<const std::string &>(), "path"_a)
        .def(py::init<const std::string&, const std::string>(), "model_path"_a, "ov_device"_a)
        .def("pychat", &Pipeline::chat,"messages"_a, "gen_config"_a, "streamer"_a = py::none())
        .def("chat_pygenerate", &Pipeline::chat_pygenerate, "messages"_a, "gen_config"_a)        
        .def("chat_pystream", &Pipeline::chat_pystream, "messages"_a, "gen_config"_a, "callback"_a, "userdata"_a)
        .def("test_pycallback",&test_pycallback)
        .def_property_readonly("model", [](const Pipeline &self) { return self.model.get(); })
        .def_property_readonly("tokenizer", [](const Pipeline &self) { return self.chatglm3_tokenizer.get(); })
        .def_readonly("prompt_tokens", &Pipeline::prompt_tokens)
        .def_readonly("completion_tokens", &Pipeline::completion_tokens);
}
