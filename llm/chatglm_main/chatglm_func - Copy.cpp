...
std::vector<int> BaseModelForCausalLM::generate(const ov::Tensor input_ids, const ov::Tensor attention_mask, const GenerationConfig &gen_config,
    ...
    // 获取下一个输出令牌
    int next_token_id = get_out_token_id(output_ids, logits, vocab_size, gen_config);
    ...
    while ((int)output_ids.size() < std::min(gen_config.max_length, n_ctx + max_new_tokens)) {
        ireq.get_tensor("input_ids").data<int>()[0] = next_token_id;
        ...
        output_ids.emplace_back(next_token_id);// 将生成的令牌添加到输出文本中

        if (streamer) {
            streamer->put({next_token_id});// 如果提供了流式处理器，则将生成的令牌放入流中
        }
...
std::pair<ov::Tensor, ov::Tensor> ChatGLM3Tokenizer::tokenize(std::string_view prompt) const {
    constexpr size_t BATCH_SIZE = 1;
...
std::pair<ov::Tensor, ov::Tensor> ChatGLM3Tokenizer::encode(const std::string &text, int max_length) const {
    ...
    auto [ids, attention_mask] = tokenize(result);
...
std::pair<ov::Tensor, ov::Tensor> ChatGLM3Tokenizer::encode_messages(const std::vector<ChatMessage> &messages, int max_length) const {
    ...
    auto [input_ids, attention_mask] = tokenize(result);
...
Pipeline::Pipeline(const std::string &openvino_model_path, const std::string& tokenizer_path, const std::string& detokenizer_path, const std::string ov_device) {
